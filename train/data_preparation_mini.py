...
def run(video_path, export_imgs = True):
    ...
    if export_imgs:
        ...
        print("Detecting teeth in frames...")
        for i, img_path in enumerate(tqdm.tqdm(img_filelist)):
            teeth_img, teeth_rect = detect_teeth(img_path)
            ...
            # Save teeth coordinates
            teeth_rect_list.append(teeth_rect)
        
        # Save all teeth rectangles to a file
        teeth_rect_array = np.array(teeth_rect_list)
        np.savetxt("{}/teeth_seg/all.txt".format(video_data_path), teeth_rect_array, fmt='%d')
