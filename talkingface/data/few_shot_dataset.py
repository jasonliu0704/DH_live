...
class Few_Shot_Dataset(data.Dataset):
    def __init__(self, dict_info, n_ref=2, is_train=False):
        ...
        if not self.driven_images:
            self.driven_images = [[]]
            self.driven_keypoints = [[]]
            self.driving_keypoints = [[]]
            self.driven_mask_keypoints = [[]]
        ...
        self.clip_count_list = [len(path) for path in self.driven_images]
        self.n_ref = n_ref
        self.n_ref5 = 5  # Define fallback
